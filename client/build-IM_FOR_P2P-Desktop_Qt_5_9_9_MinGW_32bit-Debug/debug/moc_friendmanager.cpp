/****************************************************************************
** Meta object code from reading C++ file 'friendmanager.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.9.9)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../IM_FOR_P2P/friendmanager.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'friendmanager.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.9.9. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_FriendManager_t {
    QByteArrayData data[10];
    char stringdata0[118];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_FriendManager_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_FriendManager_t qt_meta_stringdata_FriendManager = {
    {
QT_MOC_LITERAL(0, 0, 13), // "FriendManager"
QT_MOC_LITERAL(1, 14, 14), // "clickMsgSignal"
QT_MOC_LITERAL(2, 29, 0), // ""
QT_MOC_LITERAL(3, 30, 16), // "clickGroupSignal"
QT_MOC_LITERAL(4, 47, 8), // "agreeAdd"
QT_MOC_LITERAL(5, 56, 11), // "disagreeAdd"
QT_MOC_LITERAL(6, 68, 10), // "peerOnline"
QT_MOC_LITERAL(7, 79, 12), // "peerAgreeAdd"
QT_MOC_LITERAL(8, 92, 15), // "peerDisagreeAdd"
QT_MOC_LITERAL(9, 108, 9) // "peerApply"

    },
    "FriendManager\0clickMsgSignal\0\0"
    "clickGroupSignal\0agreeAdd\0disagreeAdd\0"
    "peerOnline\0peerAgreeAdd\0peerDisagreeAdd\0"
    "peerApply"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_FriendManager[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       8,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   54,    2, 0x06 /* Public */,
       3,    0,   55,    2, 0x06 /* Public */,
       4,    1,   56,    2, 0x06 /* Public */,
       5,    1,   59,    2, 0x06 /* Public */,
       6,    1,   62,    2, 0x06 /* Public */,
       7,    1,   65,    2, 0x06 /* Public */,
       8,    1,   68,    2, 0x06 /* Public */,
       9,    1,   71,    2, 0x06 /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void, QMetaType::Int,    2,
    QMetaType::Void, QMetaType::Int,    2,
    QMetaType::Void, QMetaType::Int,    2,
    QMetaType::Void, QMetaType::Int,    2,
    QMetaType::Void, QMetaType::Int,    2,
    QMetaType::Void, QMetaType::Int,    2,

       0        // eod
};

void FriendManager::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        FriendManager *_t = static_cast<FriendManager *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->clickMsgSignal(); break;
        case 1: _t->clickGroupSignal(); break;
        case 2: _t->agreeAdd((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 3: _t->disagreeAdd((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 4: _t->peerOnline((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 5: _t->peerAgreeAdd((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 6: _t->peerDisagreeAdd((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 7: _t->peerApply((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            typedef void (FriendManager::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FriendManager::clickMsgSignal)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (FriendManager::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FriendManager::clickGroupSignal)) {
                *result = 1;
                return;
            }
        }
        {
            typedef void (FriendManager::*_t)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FriendManager::agreeAdd)) {
                *result = 2;
                return;
            }
        }
        {
            typedef void (FriendManager::*_t)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FriendManager::disagreeAdd)) {
                *result = 3;
                return;
            }
        }
        {
            typedef void (FriendManager::*_t)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FriendManager::peerOnline)) {
                *result = 4;
                return;
            }
        }
        {
            typedef void (FriendManager::*_t)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FriendManager::peerAgreeAdd)) {
                *result = 5;
                return;
            }
        }
        {
            typedef void (FriendManager::*_t)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FriendManager::peerDisagreeAdd)) {
                *result = 6;
                return;
            }
        }
        {
            typedef void (FriendManager::*_t)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&FriendManager::peerApply)) {
                *result = 7;
                return;
            }
        }
    }
}

const QMetaObject FriendManager::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_FriendManager.data,
      qt_meta_data_FriendManager,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *FriendManager::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *FriendManager::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_FriendManager.stringdata0))
        return static_cast<void*>(this);
    if (!strcmp(_clname, "RelationManage"))
        return static_cast< RelationManage*>(this);
    if (!strcmp(_clname, "NetObserver"))
        return static_cast< NetObserver*>(this);
    return QWidget::qt_metacast(_clname);
}

int FriendManager::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 8)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 8;
    }
    return _id;
}

// SIGNAL 0
void FriendManager::clickMsgSignal()
{
    QMetaObject::activate(this, &staticMetaObject, 0, nullptr);
}

// SIGNAL 1
void FriendManager::clickGroupSignal()
{
    QMetaObject::activate(this, &staticMetaObject, 1, nullptr);
}

// SIGNAL 2
void FriendManager::agreeAdd(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}

// SIGNAL 3
void FriendManager::disagreeAdd(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 3, _a);
}

// SIGNAL 4
void FriendManager::peerOnline(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 4, _a);
}

// SIGNAL 5
void FriendManager::peerAgreeAdd(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 5, _a);
}

// SIGNAL 6
void FriendManager::peerDisagreeAdd(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 6, _a);
}

// SIGNAL 7
void FriendManager::peerApply(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 7, _a);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
