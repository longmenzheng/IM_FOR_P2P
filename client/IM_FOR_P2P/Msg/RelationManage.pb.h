// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: RelationManage.proto

#ifndef GOOGLE_PROTOBUF_INCLUDED_RelationManage_2eproto
#define GOOGLE_PROTOBUF_INCLUDED_RelationManage_2eproto

#include <limits>
#include <string>

#include <google/protobuf/port_def.inc>
#if PROTOBUF_VERSION < 3012000
#error This file was generated by a newer version of protoc which is
#error incompatible with your Protocol Buffer headers. Please update
#error your headers.
#endif
#if 3012003 < PROTOBUF_MIN_PROTOC_VERSION
#error This file was generated by an older version of protoc which is
#error incompatible with your Protocol Buffer headers. Please
#error regenerate this file with a newer version of protoc.
#endif

#include <google/protobuf/port_undef.inc>
#include <google/protobuf/io/coded_stream.h>
#include <google/protobuf/arena.h>
#include <google/protobuf/arenastring.h>
#include <google/protobuf/generated_message_table_driven.h>
#include <google/protobuf/generated_message_util.h>
#include <google/protobuf/inlined_string_field.h>
#include <google/protobuf/metadata_lite.h>
#include <google/protobuf/generated_message_reflection.h>
#include <google/protobuf/message.h>
#include <google/protobuf/repeated_field.h>  // IWYU pragma: export
#include <google/protobuf/extension_set.h>  // IWYU pragma: export
#include <google/protobuf/unknown_field_set.h>
// @@protoc_insertion_point(includes)
#include <google/protobuf/port_def.inc>
#define PROTOBUF_INTERNAL_EXPORT_RelationManage_2eproto
PROTOBUF_NAMESPACE_OPEN
namespace internal {
class AnyMetadata;
}  // namespace internal
PROTOBUF_NAMESPACE_CLOSE

// Internal implementation detail -- do not use these members.
struct TableStruct_RelationManage_2eproto {
  static const ::PROTOBUF_NAMESPACE_ID::internal::ParseTableField entries[]
    PROTOBUF_SECTION_VARIABLE(protodesc_cold);
  static const ::PROTOBUF_NAMESPACE_ID::internal::AuxillaryParseTableField aux[]
    PROTOBUF_SECTION_VARIABLE(protodesc_cold);
  static const ::PROTOBUF_NAMESPACE_ID::internal::ParseTable schema[1]
    PROTOBUF_SECTION_VARIABLE(protodesc_cold);
  static const ::PROTOBUF_NAMESPACE_ID::internal::FieldMetadata field_metadata[];
  static const ::PROTOBUF_NAMESPACE_ID::internal::SerializationTable serialization_table[];
  static const ::PROTOBUF_NAMESPACE_ID::uint32 offsets[];
};
extern const ::PROTOBUF_NAMESPACE_ID::internal::DescriptorTable descriptor_table_RelationManage_2eproto;
namespace IM {
class RelationManage;
class RelationManageDefaultTypeInternal;
extern RelationManageDefaultTypeInternal _RelationManage_default_instance_;
}  // namespace IM
PROTOBUF_NAMESPACE_OPEN
template<> ::IM::RelationManage* Arena::CreateMaybeMessage<::IM::RelationManage>(Arena*);
PROTOBUF_NAMESPACE_CLOSE
namespace IM {

// ===================================================================

class RelationManage PROTOBUF_FINAL :
    public ::PROTOBUF_NAMESPACE_ID::Message /* @@protoc_insertion_point(class_definition:IM.RelationManage) */ {
 public:
  inline RelationManage() : RelationManage(nullptr) {};
  virtual ~RelationManage();

  RelationManage(const RelationManage& from);
  RelationManage(RelationManage&& from) noexcept
    : RelationManage() {
    *this = ::std::move(from);
  }

  inline RelationManage& operator=(const RelationManage& from) {
    CopyFrom(from);
    return *this;
  }
  inline RelationManage& operator=(RelationManage&& from) noexcept {
    if (GetArena() == from.GetArena()) {
      if (this != &from) InternalSwap(&from);
    } else {
      CopyFrom(from);
    }
    return *this;
  }

  static const ::PROTOBUF_NAMESPACE_ID::Descriptor* descriptor() {
    return GetDescriptor();
  }
  static const ::PROTOBUF_NAMESPACE_ID::Descriptor* GetDescriptor() {
    return GetMetadataStatic().descriptor;
  }
  static const ::PROTOBUF_NAMESPACE_ID::Reflection* GetReflection() {
    return GetMetadataStatic().reflection;
  }
  static const RelationManage& default_instance();

  static void InitAsDefaultInstance();  // FOR INTERNAL USE ONLY
  static inline const RelationManage* internal_default_instance() {
    return reinterpret_cast<const RelationManage*>(
               &_RelationManage_default_instance_);
  }
  static constexpr int kIndexInFileMessages =
    0;

  friend void swap(RelationManage& a, RelationManage& b) {
    a.Swap(&b);
  }
  inline void Swap(RelationManage* other) {
    if (other == this) return;
    if (GetArena() == other->GetArena()) {
      InternalSwap(other);
    } else {
      ::PROTOBUF_NAMESPACE_ID::internal::GenericSwap(this, other);
    }
  }
  void UnsafeArenaSwap(RelationManage* other) {
    if (other == this) return;
    GOOGLE_DCHECK(GetArena() == other->GetArena());
    InternalSwap(other);
  }

  // implements Message ----------------------------------------------

  inline RelationManage* New() const final {
    return CreateMaybeMessage<RelationManage>(nullptr);
  }

  RelationManage* New(::PROTOBUF_NAMESPACE_ID::Arena* arena) const final {
    return CreateMaybeMessage<RelationManage>(arena);
  }
  void CopyFrom(const ::PROTOBUF_NAMESPACE_ID::Message& from) final;
  void MergeFrom(const ::PROTOBUF_NAMESPACE_ID::Message& from) final;
  void CopyFrom(const RelationManage& from);
  void MergeFrom(const RelationManage& from);
  PROTOBUF_ATTRIBUTE_REINITIALIZES void Clear() final;
  bool IsInitialized() const final;

  size_t ByteSizeLong() const final;
  const char* _InternalParse(const char* ptr, ::PROTOBUF_NAMESPACE_ID::internal::ParseContext* ctx) final;
  ::PROTOBUF_NAMESPACE_ID::uint8* _InternalSerialize(
      ::PROTOBUF_NAMESPACE_ID::uint8* target, ::PROTOBUF_NAMESPACE_ID::io::EpsCopyOutputStream* stream) const final;
  int GetCachedSize() const final { return _cached_size_.Get(); }

  private:
  inline void SharedCtor();
  inline void SharedDtor();
  void SetCachedSize(int size) const final;
  void InternalSwap(RelationManage* other);
  friend class ::PROTOBUF_NAMESPACE_ID::internal::AnyMetadata;
  static ::PROTOBUF_NAMESPACE_ID::StringPiece FullMessageName() {
    return "IM.RelationManage";
  }
  protected:
  explicit RelationManage(::PROTOBUF_NAMESPACE_ID::Arena* arena);
  private:
  static void ArenaDtor(void* object);
  inline void RegisterArenaDtor(::PROTOBUF_NAMESPACE_ID::Arena* arena);
  public:

  ::PROTOBUF_NAMESPACE_ID::Metadata GetMetadata() const final;
  private:
  static ::PROTOBUF_NAMESPACE_ID::Metadata GetMetadataStatic() {
    ::PROTOBUF_NAMESPACE_ID::internal::AssignDescriptors(&::descriptor_table_RelationManage_2eproto);
    return ::descriptor_table_RelationManage_2eproto.file_level_metadata[kIndexInFileMessages];
  }

  public:

  // nested types ----------------------------------------------------

  // accessors -------------------------------------------------------

  enum : int {
    kFriendNickNameFieldNumber = 7,
    kSignFieldNumber = 8,
    kNetworkTypeFieldNumber = 1,
    kMsgIDFieldNumber = 2,
    kRecvIDFieldNumber = 3,
    kSendIDFieldNumber = 4,
    kApplyerIDFieldNumber = 5,
    kPeerIDFieldNumber = 6,
  };
  // bytes friendNickName = 7;
  void clear_friendnickname();
  const std::string& friendnickname() const;
  void set_friendnickname(const std::string& value);
  void set_friendnickname(std::string&& value);
  void set_friendnickname(const char* value);
  void set_friendnickname(const void* value, size_t size);
  std::string* mutable_friendnickname();
  std::string* release_friendnickname();
  void set_allocated_friendnickname(std::string* friendnickname);
  GOOGLE_PROTOBUF_RUNTIME_DEPRECATED("The unsafe_arena_ accessors for"
  "    string fields are deprecated and will be removed in a"
  "    future release.")
  std::string* unsafe_arena_release_friendnickname();
  GOOGLE_PROTOBUF_RUNTIME_DEPRECATED("The unsafe_arena_ accessors for"
  "    string fields are deprecated and will be removed in a"
  "    future release.")
  void unsafe_arena_set_allocated_friendnickname(
      std::string* friendnickname);
  private:
  const std::string& _internal_friendnickname() const;
  void _internal_set_friendnickname(const std::string& value);
  std::string* _internal_mutable_friendnickname();
  public:

  // bytes sign = 8;
  void clear_sign();
  const std::string& sign() const;
  void set_sign(const std::string& value);
  void set_sign(std::string&& value);
  void set_sign(const char* value);
  void set_sign(const void* value, size_t size);
  std::string* mutable_sign();
  std::string* release_sign();
  void set_allocated_sign(std::string* sign);
  GOOGLE_PROTOBUF_RUNTIME_DEPRECATED("The unsafe_arena_ accessors for"
  "    string fields are deprecated and will be removed in a"
  "    future release.")
  std::string* unsafe_arena_release_sign();
  GOOGLE_PROTOBUF_RUNTIME_DEPRECATED("The unsafe_arena_ accessors for"
  "    string fields are deprecated and will be removed in a"
  "    future release.")
  void unsafe_arena_set_allocated_sign(
      std::string* sign);
  private:
  const std::string& _internal_sign() const;
  void _internal_set_sign(const std::string& value);
  std::string* _internal_mutable_sign();
  public:

  // int32 networkType = 1;
  void clear_networktype();
  ::PROTOBUF_NAMESPACE_ID::int32 networktype() const;
  void set_networktype(::PROTOBUF_NAMESPACE_ID::int32 value);
  private:
  ::PROTOBUF_NAMESPACE_ID::int32 _internal_networktype() const;
  void _internal_set_networktype(::PROTOBUF_NAMESPACE_ID::int32 value);
  public:

  // int32 msgID = 2;
  void clear_msgid();
  ::PROTOBUF_NAMESPACE_ID::int32 msgid() const;
  void set_msgid(::PROTOBUF_NAMESPACE_ID::int32 value);
  private:
  ::PROTOBUF_NAMESPACE_ID::int32 _internal_msgid() const;
  void _internal_set_msgid(::PROTOBUF_NAMESPACE_ID::int32 value);
  public:

  // int32 recvID = 3;
  void clear_recvid();
  ::PROTOBUF_NAMESPACE_ID::int32 recvid() const;
  void set_recvid(::PROTOBUF_NAMESPACE_ID::int32 value);
  private:
  ::PROTOBUF_NAMESPACE_ID::int32 _internal_recvid() const;
  void _internal_set_recvid(::PROTOBUF_NAMESPACE_ID::int32 value);
  public:

  // int32 sendID = 4;
  void clear_sendid();
  ::PROTOBUF_NAMESPACE_ID::int32 sendid() const;
  void set_sendid(::PROTOBUF_NAMESPACE_ID::int32 value);
  private:
  ::PROTOBUF_NAMESPACE_ID::int32 _internal_sendid() const;
  void _internal_set_sendid(::PROTOBUF_NAMESPACE_ID::int32 value);
  public:

  // int32 applyerID = 5;
  void clear_applyerid();
  ::PROTOBUF_NAMESPACE_ID::int32 applyerid() const;
  void set_applyerid(::PROTOBUF_NAMESPACE_ID::int32 value);
  private:
  ::PROTOBUF_NAMESPACE_ID::int32 _internal_applyerid() const;
  void _internal_set_applyerid(::PROTOBUF_NAMESPACE_ID::int32 value);
  public:

  // int32 peerID = 6;
  void clear_peerid();
  ::PROTOBUF_NAMESPACE_ID::int32 peerid() const;
  void set_peerid(::PROTOBUF_NAMESPACE_ID::int32 value);
  private:
  ::PROTOBUF_NAMESPACE_ID::int32 _internal_peerid() const;
  void _internal_set_peerid(::PROTOBUF_NAMESPACE_ID::int32 value);
  public:

  // @@protoc_insertion_point(class_scope:IM.RelationManage)
 private:
  class _Internal;

  template <typename T> friend class ::PROTOBUF_NAMESPACE_ID::Arena::InternalHelper;
  typedef void InternalArenaConstructable_;
  typedef void DestructorSkippable_;
  ::PROTOBUF_NAMESPACE_ID::internal::ArenaStringPtr friendnickname_;
  ::PROTOBUF_NAMESPACE_ID::internal::ArenaStringPtr sign_;
  ::PROTOBUF_NAMESPACE_ID::int32 networktype_;
  ::PROTOBUF_NAMESPACE_ID::int32 msgid_;
  ::PROTOBUF_NAMESPACE_ID::int32 recvid_;
  ::PROTOBUF_NAMESPACE_ID::int32 sendid_;
  ::PROTOBUF_NAMESPACE_ID::int32 applyerid_;
  ::PROTOBUF_NAMESPACE_ID::int32 peerid_;
  mutable ::PROTOBUF_NAMESPACE_ID::internal::CachedSize _cached_size_;
  friend struct ::TableStruct_RelationManage_2eproto;
};
// ===================================================================


// ===================================================================

#ifdef __GNUC__
  #pragma GCC diagnostic push
  #pragma GCC diagnostic ignored "-Wstrict-aliasing"
#endif  // __GNUC__
// RelationManage

// int32 networkType = 1;
inline void RelationManage::clear_networktype() {
  networktype_ = 0;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::_internal_networktype() const {
  return networktype_;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::networktype() const {
  // @@protoc_insertion_point(field_get:IM.RelationManage.networkType)
  return _internal_networktype();
}
inline void RelationManage::_internal_set_networktype(::PROTOBUF_NAMESPACE_ID::int32 value) {
  
  networktype_ = value;
}
inline void RelationManage::set_networktype(::PROTOBUF_NAMESPACE_ID::int32 value) {
  _internal_set_networktype(value);
  // @@protoc_insertion_point(field_set:IM.RelationManage.networkType)
}

// int32 msgID = 2;
inline void RelationManage::clear_msgid() {
  msgid_ = 0;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::_internal_msgid() const {
  return msgid_;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::msgid() const {
  // @@protoc_insertion_point(field_get:IM.RelationManage.msgID)
  return _internal_msgid();
}
inline void RelationManage::_internal_set_msgid(::PROTOBUF_NAMESPACE_ID::int32 value) {
  
  msgid_ = value;
}
inline void RelationManage::set_msgid(::PROTOBUF_NAMESPACE_ID::int32 value) {
  _internal_set_msgid(value);
  // @@protoc_insertion_point(field_set:IM.RelationManage.msgID)
}

// int32 recvID = 3;
inline void RelationManage::clear_recvid() {
  recvid_ = 0;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::_internal_recvid() const {
  return recvid_;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::recvid() const {
  // @@protoc_insertion_point(field_get:IM.RelationManage.recvID)
  return _internal_recvid();
}
inline void RelationManage::_internal_set_recvid(::PROTOBUF_NAMESPACE_ID::int32 value) {
  
  recvid_ = value;
}
inline void RelationManage::set_recvid(::PROTOBUF_NAMESPACE_ID::int32 value) {
  _internal_set_recvid(value);
  // @@protoc_insertion_point(field_set:IM.RelationManage.recvID)
}

// int32 sendID = 4;
inline void RelationManage::clear_sendid() {
  sendid_ = 0;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::_internal_sendid() const {
  return sendid_;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::sendid() const {
  // @@protoc_insertion_point(field_get:IM.RelationManage.sendID)
  return _internal_sendid();
}
inline void RelationManage::_internal_set_sendid(::PROTOBUF_NAMESPACE_ID::int32 value) {
  
  sendid_ = value;
}
inline void RelationManage::set_sendid(::PROTOBUF_NAMESPACE_ID::int32 value) {
  _internal_set_sendid(value);
  // @@protoc_insertion_point(field_set:IM.RelationManage.sendID)
}

// int32 applyerID = 5;
inline void RelationManage::clear_applyerid() {
  applyerid_ = 0;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::_internal_applyerid() const {
  return applyerid_;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::applyerid() const {
  // @@protoc_insertion_point(field_get:IM.RelationManage.applyerID)
  return _internal_applyerid();
}
inline void RelationManage::_internal_set_applyerid(::PROTOBUF_NAMESPACE_ID::int32 value) {
  
  applyerid_ = value;
}
inline void RelationManage::set_applyerid(::PROTOBUF_NAMESPACE_ID::int32 value) {
  _internal_set_applyerid(value);
  // @@protoc_insertion_point(field_set:IM.RelationManage.applyerID)
}

// int32 peerID = 6;
inline void RelationManage::clear_peerid() {
  peerid_ = 0;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::_internal_peerid() const {
  return peerid_;
}
inline ::PROTOBUF_NAMESPACE_ID::int32 RelationManage::peerid() const {
  // @@protoc_insertion_point(field_get:IM.RelationManage.peerID)
  return _internal_peerid();
}
inline void RelationManage::_internal_set_peerid(::PROTOBUF_NAMESPACE_ID::int32 value) {
  
  peerid_ = value;
}
inline void RelationManage::set_peerid(::PROTOBUF_NAMESPACE_ID::int32 value) {
  _internal_set_peerid(value);
  // @@protoc_insertion_point(field_set:IM.RelationManage.peerID)
}

// bytes friendNickName = 7;
inline void RelationManage::clear_friendnickname() {
  friendnickname_.ClearToEmpty(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), GetArena());
}
inline const std::string& RelationManage::friendnickname() const {
  // @@protoc_insertion_point(field_get:IM.RelationManage.friendNickName)
  return _internal_friendnickname();
}
inline void RelationManage::set_friendnickname(const std::string& value) {
  _internal_set_friendnickname(value);
  // @@protoc_insertion_point(field_set:IM.RelationManage.friendNickName)
}
inline std::string* RelationManage::mutable_friendnickname() {
  // @@protoc_insertion_point(field_mutable:IM.RelationManage.friendNickName)
  return _internal_mutable_friendnickname();
}
inline const std::string& RelationManage::_internal_friendnickname() const {
  return friendnickname_.Get();
}
inline void RelationManage::_internal_set_friendnickname(const std::string& value) {
  
  friendnickname_.Set(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), value, GetArena());
}
inline void RelationManage::set_friendnickname(std::string&& value) {
  
  friendnickname_.Set(
    &::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), ::std::move(value), GetArena());
  // @@protoc_insertion_point(field_set_rvalue:IM.RelationManage.friendNickName)
}
inline void RelationManage::set_friendnickname(const char* value) {
  GOOGLE_DCHECK(value != nullptr);
  
  friendnickname_.Set(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), ::std::string(value),
              GetArena());
  // @@protoc_insertion_point(field_set_char:IM.RelationManage.friendNickName)
}
inline void RelationManage::set_friendnickname(const void* value,
    size_t size) {
  
  friendnickname_.Set(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), ::std::string(
      reinterpret_cast<const char*>(value), size), GetArena());
  // @@protoc_insertion_point(field_set_pointer:IM.RelationManage.friendNickName)
}
inline std::string* RelationManage::_internal_mutable_friendnickname() {
  
  return friendnickname_.Mutable(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), GetArena());
}
inline std::string* RelationManage::release_friendnickname() {
  // @@protoc_insertion_point(field_release:IM.RelationManage.friendNickName)
  return friendnickname_.Release(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), GetArena());
}
inline void RelationManage::set_allocated_friendnickname(std::string* friendnickname) {
  if (friendnickname != nullptr) {
    
  } else {
    
  }
  friendnickname_.SetAllocated(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), friendnickname,
      GetArena());
  // @@protoc_insertion_point(field_set_allocated:IM.RelationManage.friendNickName)
}
inline std::string* RelationManage::unsafe_arena_release_friendnickname() {
  // @@protoc_insertion_point(field_unsafe_arena_release:IM.RelationManage.friendNickName)
  GOOGLE_DCHECK(GetArena() != nullptr);
  
  return friendnickname_.UnsafeArenaRelease(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(),
      GetArena());
}
inline void RelationManage::unsafe_arena_set_allocated_friendnickname(
    std::string* friendnickname) {
  GOOGLE_DCHECK(GetArena() != nullptr);
  if (friendnickname != nullptr) {
    
  } else {
    
  }
  friendnickname_.UnsafeArenaSetAllocated(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(),
      friendnickname, GetArena());
  // @@protoc_insertion_point(field_unsafe_arena_set_allocated:IM.RelationManage.friendNickName)
}

// bytes sign = 8;
inline void RelationManage::clear_sign() {
  sign_.ClearToEmpty(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), GetArena());
}
inline const std::string& RelationManage::sign() const {
  // @@protoc_insertion_point(field_get:IM.RelationManage.sign)
  return _internal_sign();
}
inline void RelationManage::set_sign(const std::string& value) {
  _internal_set_sign(value);
  // @@protoc_insertion_point(field_set:IM.RelationManage.sign)
}
inline std::string* RelationManage::mutable_sign() {
  // @@protoc_insertion_point(field_mutable:IM.RelationManage.sign)
  return _internal_mutable_sign();
}
inline const std::string& RelationManage::_internal_sign() const {
  return sign_.Get();
}
inline void RelationManage::_internal_set_sign(const std::string& value) {
  
  sign_.Set(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), value, GetArena());
}
inline void RelationManage::set_sign(std::string&& value) {
  
  sign_.Set(
    &::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), ::std::move(value), GetArena());
  // @@protoc_insertion_point(field_set_rvalue:IM.RelationManage.sign)
}
inline void RelationManage::set_sign(const char* value) {
  GOOGLE_DCHECK(value != nullptr);
  
  sign_.Set(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), ::std::string(value),
              GetArena());
  // @@protoc_insertion_point(field_set_char:IM.RelationManage.sign)
}
inline void RelationManage::set_sign(const void* value,
    size_t size) {
  
  sign_.Set(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), ::std::string(
      reinterpret_cast<const char*>(value), size), GetArena());
  // @@protoc_insertion_point(field_set_pointer:IM.RelationManage.sign)
}
inline std::string* RelationManage::_internal_mutable_sign() {
  
  return sign_.Mutable(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), GetArena());
}
inline std::string* RelationManage::release_sign() {
  // @@protoc_insertion_point(field_release:IM.RelationManage.sign)
  return sign_.Release(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), GetArena());
}
inline void RelationManage::set_allocated_sign(std::string* sign) {
  if (sign != nullptr) {
    
  } else {
    
  }
  sign_.SetAllocated(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(), sign,
      GetArena());
  // @@protoc_insertion_point(field_set_allocated:IM.RelationManage.sign)
}
inline std::string* RelationManage::unsafe_arena_release_sign() {
  // @@protoc_insertion_point(field_unsafe_arena_release:IM.RelationManage.sign)
  GOOGLE_DCHECK(GetArena() != nullptr);
  
  return sign_.UnsafeArenaRelease(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(),
      GetArena());
}
inline void RelationManage::unsafe_arena_set_allocated_sign(
    std::string* sign) {
  GOOGLE_DCHECK(GetArena() != nullptr);
  if (sign != nullptr) {
    
  } else {
    
  }
  sign_.UnsafeArenaSetAllocated(&::PROTOBUF_NAMESPACE_ID::internal::GetEmptyStringAlreadyInited(),
      sign, GetArena());
  // @@protoc_insertion_point(field_unsafe_arena_set_allocated:IM.RelationManage.sign)
}

#ifdef __GNUC__
  #pragma GCC diagnostic pop
#endif  // __GNUC__

// @@protoc_insertion_point(namespace_scope)

}  // namespace IM

// @@protoc_insertion_point(global_scope)

#include <google/protobuf/port_undef.inc>
#endif  // GOOGLE_PROTOBUF_INCLUDED_GOOGLE_PROTOBUF_INCLUDED_RelationManage_2eproto
